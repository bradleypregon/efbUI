...
struct SimbriefScreen: View {
  ...
  func createRoute(navlog: [OFPNavlog]) {
    ...
    for nav in navlog {
      ...
      //let wpt: MyWaypoint = MyWaypoint(identifier: nav.ident, name: nav.name, lat: Double(nav.lat) ?? .zero, long: Double(nav.long) ?? .zero, type: .airport)
      
      
      //routeManager.waypoints.append(wpt)
    }
  }
}
